...
export function renderPublicMenuHTML(res, categories, uncategorized, settings, tableName, info) {
  const themeStyles = `
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .menu-wrapper { max-width: 1200px; margin: 0 auto; position: relative; }
    .restaurant-header {
    ...
    .restaurant-logo { max-height: 100px; max-width: 200px; object-fit: contain; margin-bottom: 15px; position: relative; z-index: 1; }
    .restaurant-name {
      font-size: 2.5rem;
      font-weight: 700;
      color: var(--primary);
    ...
    .header-actions { display: flex; gap: 15px; justify-content: center; flex-wrap: wrap; margin-top: 20px; }
    .action-btn {
      padding: 10px 25px; border-radius: 50px; text-decoration: none; color: white; font-weight: 600;
    ...
    .search-box {
      width: 100%; padding: 15px 25px; border: none; border-radius: 50px; font-size: 1rem;
      box-shadow: var(--card-shadow); transition: var(--transition); background: rgba(255,255,255,0.95);
    }
    .search-box:focus { outline: none; box-shadow: var(--hover-shadow); transform: scale(1.02); }
    .category-section { margin-bottom: 50px; }
    .category-title {
      font-size: 2rem; font-weight: 600; color: white; margin-bottom: 20px; padding-right: 15px;
      position: relative; text-shadow: 2px 2px 4px rgba(0,0,0,0.2);
    }
    .category-title::after {
      content: ''; position: absolute; right: 0; bottom: -5px; width: 60px; height: 4px;
      background: white; border-radius: 2px;
    }
    .items-grid {
      display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 25px; padding: 10px;
    }
    .item-card {
      background: rgba(255,255,255,0.95); backdrop-filter: blur(10px); border-radius: 20px; overflow: hidden;
      transition: var(--transition); box-shadow: var(--card-shadow); position: relative; cursor: pointer;
      animation: fadeIn 0.5s ease;
    }
    @keyframes fadeIn { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
    .item-card:hover { transform: translateY(-10px); box-shadow: var(--hover-shadow); }
    .item-card.featured { border: 2px solid gold; position: relative; }
    .featured-badge {
      position: absolute; top: 15px; left: 15px; background: gold; color: #333;
      width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center;
      justify-content: center; font-size: 1.2rem; box-shadow: 0 5px 10px rgba(0,0,0,0.2); z-index: 2;
    }
    .item-image { width: 100%; height: 200px; object-fit: cover; transition: var(--transition); }
    .item-card:hover .item-image { transform: scale(1.05); }
    .item-content { padding: 20px; }
    .item-name { font-size: 1.3rem; font-weight: 600; color: #333; margin-bottom: 10px; }
    .item-price { color: green; font-weight: 700; font-size: 1.2rem; margin-bottom: 15px; }
    .add-to-cart-btn {
      width: 100%; padding: 12px; background: var(--primary); color: white; border: none;
      border-radius: 10px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: var(--transition);
      display: flex; align-items: center; justify-content: center; gap: 5px;
    }
    .add-to-cart-btn:hover { background: var(--secondary); transform: translateY(-2px); }
    .floating-cart { position: fixed; bottom: 20px; left: 20px; z-index: 1000; }
    .cart-toggle {
      width: 60px; height: 60px; border-radius: 50%; background: linear-gradient(135deg, var(--primary), var(--secondary));
      color: white; border: none; cursor: pointer; font-size: 1.5rem; box-shadow: 0 5px 20px rgba(0,0,0,0.3);
      transition: var(--transition); display: flex; align-items: center; justify-content: center;
    }
    .cart-toggle:hover { transform: scale(1.1) rotate(10deg); }
    .cart-count {
      position: absolute; top: -5px; right: -5px; background: red; color: white; width: 25px; height: 25px;
      border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.9rem; font-weight: bold;
    }
    .cart-panel {
      position: fixed; bottom: 90px; left: 20px; width: 320px; max-height: 400px; background: white;
      border-radius: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.3); padding: 20px; overflow-y: auto; display: none;
    }
    .cart-panel.show { display: block; animation: slideUp 0.3s ease; }
    @keyframes slideUp { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
    .cart-item { display: flex; justify-content: space-between; align-items: center; padding: 10px 0; border-bottom: 1px solid #eee; }
    .cart-item-details { flex: 1; }
    .cart-item-name { font-weight: 600; font-size: 0.95rem; }
    .cart-item-options { font-size: 0.8rem; color: #666; }
    .cart-item-actions { display: flex; gap: 5px; }
    .cart-item-actions button { width: 30px; height: 30px; border: none; border-radius: 5px; cursor: pointer; font-weight: bold; }
    .cart-total { font-weight: 700; font-size: 1.2rem; color: var(--primary); padding: 10px 0; }
    .whatsapp-btn {
      width: 100%; padding: 15px; background: #25D366; color: white; border: none; border-radius: 10px;
      font-weight: 600; cursor: pointer; transition: var(--transition); margin-top: 10px;
    }
    .whatsapp-btn:hover { background: #128C7E; }
    .options-modal {
      display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
      background: rgba(0,0,0,0.5); justify-content: center; align-items: center; z-index: 2000; backdrop-filter: blur(5px);
    }
    .options-modal.show { display: flex; }
    .modal-content {
      background: white; padding: 30px; border-radius: 20px; width: 90%; max-width: 400px;
      max-height: 80vh; overflow-y: auto; animation: modalPop 0.3s ease;
    }
    @keyframes modalPop { from { opacity: 0; transform: scale(0.9); } to { opacity: 1; transform: scale(1); } }
    .modal-title { font-size: 1.5rem; color: var(--primary); margin-bottom: 20px; }
    ...
    @media (max-width: 768px) {
      ...
      .cart-panel { width: calc(100% - 40px); left: 20px; }
      .floating-cart { bottom: 15px; left: 15px; }
      .cart-toggle { width: 50px; height: 50px; font-size: 1.3rem; }
    }
    @media (max-width: 480px) {
      body { padding: 10px; }
      .restaurant-header { padding: 20px 15px; }
...
export function renderAboutHTML(res, info, settings) {
  return `<!DOCTYPE html>
<html dir="rtl">
<head>
  <meta charset="UTF-8">
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .logo { max-height: 80px; max-width: 100%; object-fit: contain; }
    h1 { color: var(--primary); margin: 10px 0; }
    .info-card { background: #f9f9f9; padding: 20px; border-radius: 10px; margin-bottom: 25px; }
    .info-item {
      display: flex; align-items: center; gap: 15px; padding: 12px 0; border-bottom: 1px solid #eee;
    }
    .info-item:last-child { border-bottom: none; }
    .info-icon { font-size: 1.5rem; min-width: 40px; text-align: center; }
    .info-content { flex: 1; }
    .info-label { font-weight: bold; color: #555; font-size: 0.9rem; }
    .info-value { font-size: 1.1rem; }
    .map-container { margin: 25px 0; border-radius: 10px; overflow: hidden; box-shadow: 0 2px 5px rgba(0,0,0,0.1); }
    ...
    .social-link {
      ...
      border-radius: 50%; background: var(--primary); color: white; text-decoration: none; font-size: 1.5rem;
      transition: transform 0.2s;
    }
    .social-link:hover { transform: scale(1.1); }
    .back-link {
      display: block; text-align: center; margin-top: 25px; padding: 12px; background: var(--primary);
